...
if __name__ == '__main__':
    ...
    if arg.informed:
        information = data_augmentation.get_information()
    else: information = arg.informed
    ...
    elif not arg.constrain:
        chosen_intervals ={
            'radius': training_interval['radius']['medium'],
            'distance':  training_interval['distance']['medium'],
            'sigma_radius': training_interval['sigma_radius']['test'],
            'omega_distance':  training_interval['omega_distance']['medium']}   
    else:
        chosen_intervals ={
    ...
    constrains = {'fast_sim': arg.fast_sim, 'samples': None, 'check': arg.check, 'constrain':arg.constrain}

    # create simulation object with constrains
    if arg.train: simulation = simulation_dataloader.Simulation(path=path_simulation_data, sim_source=arg.sim_source,
                                                  constrains=constrains, intervals=chosen_intervals)
    #utilities.record_simulation_parameters
    if arg.test:
        # create experiment object
        print("Initializing experiment object")
        ...
        if arg.fast_exp:
            ...
            print("Fitting experiment images")
    ...
    if arg.train:
        ...
        #     validation_data = experiment.get_type('400K', experiment_images, experiment_target_values, sample=None)
        #     #validation_data = experiment.get_type('400K', experiment_images, experiment_target_values, sample=5 if arg.check else 25)
    ...
    deep_learning_parameter = {'algorithm': arg.algorithm, 'path': path_project, 'train': arg.train,'validation': arg.validation, 'estimation': arg.estimation,
                               'morphology': arg.morphology, 'distribution': arg.distr, 'beta': arg.beta, 'run':arg.run, 'check': arg.check, 'loss':arg.loss, 'informed': information} #arg.informed}
    if arg.train: image_shape= simulation_images[0].shape
    else: image_shape = experiment_images[0].shape
    algorithm = intialize_algorithm(algorithm=arg.algorithm, input_shape=(image_shape + (1,)), parameter=deep_learning_parameter, output_units=output_units)
    if arg.train:
        ...
        # The case if we only use the network on the test images
        algorithm.test_on_experiment_in_batch(experiment_images, experiment_target_values)
    ...
    utilities.record_run_parameters(desc='',run_path=algorithm.model_path, morphology=algorithm.keys, training_scope=chosen_intervals, 
                                    ...
                                    decay=algorithm.model.optimizer.weight_decay, informed=arg.informed, constrain=arg.constrain, shuffled=algorithm.shuffle)#alty_0.00001_informed_0.0
    # 'penalty-1_0.00001'