...
class AlgorithmBase:
    ...
    def train_on_simulations(self, images, target_values, validation_data):
        ...
        for i in range(MAX_VALIDATION_ROUND):
            ...
            history = self.model.fit(x=images, y=labels_training_list, callbacks=[h5_logger_callback, csv_logger_callback], epochs=num_epochs, batch_size=256, shuffle=False)
    ...
    def save_prediction(self, target_values, target_values_prediction, model_path, data):
        pass
        #prediction_max, prediction_avg = self.get_numerical_prediction(target_values_prediction)
    ...
    def test_on_experiment(self, images, target_values):
        ...
        if self.model_compiled and self.model_path:
            ...
            for i in range(MAX_VALIDATION_ROUND):
               ...
               targets_test_prediction = self.get_numerical_prediction(self.model.predict(images))
    ...
    def get_MSE(self, values, targets, keys=None):
        # recursive function
        ...
        if not keys: keys = self.keys_validation
